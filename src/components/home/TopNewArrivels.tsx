...
const TopNewArrivals = ({newProducts}: {newProducts: UseQueryResult<Product[], unknown>}) => {
      ...
      <Box>
        ...
        <Grid container spacing={2} sx={{ mt: "1rem" }}>
          <Grid container item xs={12} spacing={2}>
            {newProducts.data && newProducts.data.map((product, index) => (
              <Grid
                item
              ...
              >
                ...
                >
                  ...
                  <CardContent sx={{ pt: 0, zIndex: 4,display: "flex", flexDirection: "column", flex: 1 }}>
                    <Link href={`/product/${product._id}`}>
                      <Typography
                        variant="subtitle1"
                    ...
                    <Box sx={{pt: 2}}>
                      <Rating name="read-only" value={countRating(product.reviews)} readOnly size="small"/>
                    </Box>
